...
pkg.$sleep = function() {
    ...
    r.open('GET', window.location + (i > 0 ? "&" : "?") + t, true); // *&* r.open('GET', window.location + (i > 0 ? "&" : "?") + t, false);
    r.send(null);
};
...
$Request.prototype.open = function(method, url, async, user, password) {
        ...
        (new zebra.URL(url)).host.toLowerCase() == location.host.toLowerCase())
    {
        ...
        return this._request.open(method, url, true, user, password);
        // *&* return this._request.open(method, url, (async !== false), user, password);
    }
    else {
        this._xdomain = true;
        this._async = true; // *&* this._async = (async === true);
