...
class TokenSpot {
	...
	clickAction(obj) {
		if (G.builder.enabled) {
			...
			G.builder.currentTokenSpot = this;
		}
		this.update();
	...
	deleteTokenSpot() {
		if(G.builder.enabled) {
			G.builder.currentTokenSpot.sprite.x = -1000;
			G.builder.currentTokenSpot.sprite.y = -1000;
		}
	}
...
class RegionGraphic {
	...
	clickAction(obj) {
		...
		if(G.builder.enabled) {
			if(G.builder.seaConnOn) // if this is true, a region has already been clicked and then "started"
			{
				G.builder.newConnList.push(this.P.name);
				this.P.seaConn.push(G.builder.currentRegion.name);
				//G.bmregionNameElement.innerHTML = "Land Connection #" + G.builder.newConnList.length +" : " + G.buildModeRegion.name + " -> " + this.P.name;
			}
			else {
				G.builder.currentRegion = this.P;
	...
	updateConnectionLines() {
		...
		{
			regTemp = findRegByName(G.GV.regions, this.P.landConn[fl])
			if(regTemp != null)
			{
		...
		{
			regTemp = findRegByName(G.GV.regions, this.P.seaConn[fs])
			if(regTemp != null)
			{
				reg2 = regTemp.regionGraphic;
